...
using MRShop.Migrations.SeedData;
...
namespace MRShop.Tests
{
    public abstract class MRShopTestBase : AbpIntegratedTestBase<MRShopTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MRShopTestBase()
        {
        ...
        protected void UsingDbContext(Action<MRShopDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MRShopDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MRShopDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MRShopDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MRShopDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MRShopDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MRShopDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MRShopDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MRShopDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MRShopDbContext, Task<T>> func)
        {
            T result;
